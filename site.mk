...
# pubspec.yaml now lives at packages/betto_onnxrt/pubspec.yaml after the
# monorepo restructure.
PKG_NAME    := $(shell awk '/^name:/{print $$2}'        packages/betto_onnxrt/pubspec.yaml)
PKG_DESC    := $(shell awk '/^description:/{print $$2}' packages/betto_onnxrt/pubspec.yaml)
PKG_VERSION := $(shell awk '/^version:/{print $$2}'     packages/betto_onnxrt/pubspec.yaml)
REPO_URL    := $(shell awk '/^repository:/{print $$2}'  packages/betto_onnxrt/pubspec.yaml)
...
$(_HEADER): $(DOCS_DIR)/template/header.html packages/betto_onnxrt/pubspec.yaml | $(SITE_DIR)
	sed -e 's|\$$name\$$|$(PKG_NAME)|g' \
	    -e 's|\$$version\$$|$(PKG_VERSION)|g' \
	    -e 's|\$$repo\$$|$(REPO_URL)|g' \
	    $< > $@

$(_INDEX): $(DOCS_DIR)/index.md packages/betto_onnxrt/pubspec.yaml | $(SITE_DIR)
	sed -e 's|\$$name\$$|$(PKG_NAME)|g' \
	    -e 's|\$$version\$$|$(PKG_VERSION)|g' \
	    -e 's|\$$repo\$$|$(REPO_URL)|g' \
...
$(SITE_DIR)/index.html: $(_INDEX) $(DOCS_DIR)/.pandoc $(_HEADER) | $(SITE_DIR)
	pandoc --defaults="$(DOCS_DIR)/.pandoc" $(_INDEX) packages/betto_onnxrt/README.md -o "$(SITE_DIR)/index.html";
...
$(SITE_DIR)/api/index.html: packages/betto_onnxrt/lib/** packages/betto_onnxrt/test/** | $(SITE_DIR)
	cd packages/betto_onnxrt && dart doc -o ../../$(SITE_DIR)/api/index.html
